	...
	<div class="container guide-bg-none">
		<div class="guide-wrap">
			<div class="guide-dropdown-outer guide-responsive">
				...
				<?php endif; ?>
				<?php
				$show_rescan_btn = ($page === 'myworks-wc-xero-sync-settings' || $page === 'myworks-wc-xero-sync-map');
				if ($show_rescan_btn):
					?>
					<!-- Rescan Xero Data Dropdown -->
					<input type="hidden" id="mwxs_rescan_nonce"
						value="<?php echo esc_attr(wp_create_nonce('myworks_wc_xero_sync_rescan_data')); ?>">
					<div class="aoutomated-outer g-d-o-btn" id="mwxs_rescan_dropdown_wrap" <?php if ($page === 'myworks-wc-xero-sync-settings' || ($page === 'myworks-wc-xero-sync-map' && in_array($tab, array('custom-fields', 'tax-class'), true)))
						echo 'style="margin-right:20px;"'; ?>>
						<div class="col col-m auto-map-btn">
							<span class="dropbtn col-m-btn"
								id="mwxs_rescan_btn"><?php esc_html_e('Rescan Xero Data', 'myworks-sync-for-xero'); ?> <i
									class="fa fa-angle-down"></i></span>
							<div class="dropdown-content">
								<ul class="guide-accordion">
									<li><a href="javascript:void(0);" class="mwxs-rescan-item"
											data-type="xero_products"><?php esc_html_e('Rescan Products', 'myworks-sync-for-xero'); ?></a>
									</li>
									<li><a href="javascript:void(0);" class="mwxs-rescan-item"
											data-type="xero_customers"><?php esc_html_e('Rescan Customers', 'myworks-sync-for-xero'); ?></a>
									</li>
									<li><a href="javascript:void(0);" class="mwxs-rescan-item"
											data-type="xero_data"><?php esc_html_e('Rescan Other Lists', 'myworks-sync-for-xero'); ?></a>
									</li>
								</ul>
							</div>
						</div>
					...
					<!-- Rescan Result Modal -->
					<div id="mwxs_rescan_modal_overlay"
						style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.5); z-index:999999;">
						<div id="mwxs_rescan_modal"
							style="position:absolute; top:50%; left:50%; transform:translate(-50%,-50%); background:#fff; border-radius:6px; padding:30px 35px; min-width:280px; text-align:center; box-shadow:0 4px 20px rgba(0,0,0,0.3);">
							<div id="mwxs_rescan_modal_icon" style="font-size:42px; color:#2196F3; margin-bottom:10px;">&#10003;
							</div>
							<h3 style="margin:0 0 15px; color:#333; font-size:18px;" id="mwxs_rescan_modal_title">
								<?php esc_html_e('Rescan Complete!', 'myworks-sync-for-xero'); ?>
							</h3>
							<div id="mwxs_rescan_modal_body" style="font-size:15px; color:#555; line-height:1.8;"></div>
							<button id="mwxs_rescan_modal_close"
								style="margin-top:20px; background:#017cb7; color:#fff; border:none; padding:10px 28px; border-radius:4px; font-size:14px; cursor:pointer;"><?php esc_html_e('OK', 'myworks-sync-for-xero'); ?></button>
						</div>
					</div>
					<script type="text/javascript">
						jQuery(document).ready(function ($) {
							$('.mwxs-rescan-item').on('click', function () {
								var type = $(this).data('type');
								var $btn = $('#mwxs_rescan_btn');
								...
								jQuery.ajax({
									...
									data: {
										...
										rescan_nonce: $('#mwxs_rescan_nonce').val(),
										rescan_type: type
									},
									success: function (response) {
										...
										if (response && response.success) {
											...
											$('#mwxs_rescan_modal_icon').html('&#10003;').css('color', '#2196F3');
											$('#mwxs_rescan_modal_title').text('<?php echo esc_js(__('Rescan Complete!', 'myworks-sync-for-xero')); ?>');
											$('#mwxs_rescan_modal_body').html(data.message || '');
											$('#mwxs_rescan_modal_overlay').fadeIn(200);
											if (data.reload) {
											...
											$('#mwxs_rescan_modal_icon').html('&#10007;').css('color', '#e53935');
											$('#mwxs_rescan_modal_title').text('<?php echo esc_js(__('Rescan Failed', 'myworks-sync-for-xero')); ?>');
											$('#mwxs_rescan_modal_body').html(errMsg);
											$('#mwxs_rescan_modal_overlay').fadeIn(200);
										}
									},
									error: function () {
										...
										$('#mwxs_rescan_modal_icon').html('&#10007;').css('color', '#e53935');
										$('#mwxs_rescan_modal_title').text('<?php echo esc_js(__('Rescan Failed', 'myworks-sync-for-xero')); ?>');
										$('#mwxs_rescan_modal_body').html('<?php echo esc_js(__('An error occurred. Please try again.', 'myworks-sync-for-xero')); ?>');
										$('#mwxs_rescan_modal_overlay').fadeIn(200);
									}
								});
							});

							$('#mwxs_rescan_modal_close, #mwxs_rescan_modal_overlay').on('click', function (e) {
								if (e.target === this) $('#mwxs_rescan_modal_overlay').fadeOut(200);
							});
						});
					</script>
				<?php endif; ?>
				...
				<?php if ($customer_map_page): ?>
					<div class="aoutomated-outer g-d-o-btn" style="margin-right:20px;">
						<div class="col col-m auto-map-btn">
							...
							<div class="dropdown-content wqam_ndc">
								<table class="wqam_tbl">
									<tr>
										<td width="50%"><?php esc_html_e('WooCommerce Field', 'myworks-sync-for-xero'); ?> :
										</td>
										<td>
									...
									<tr>
										<td><?php esc_html_e('Xero Field', 'myworks-sync-for-xero'); ?> :</td>
										<td>
									...
									<tr>
										<td colspan="2">
											...
											&nbsp;
											<?php esc_html_e('Only apply to unmapped customers', 'myworks-sync-for-xero'); ?>
										</td>
									</tr>
								...
								<table class="wqam_tbl">
									...
									<tr>
										<td colspan="2">
											...
											&nbsp;
											<?php esc_html_e('Only apply to unmapped products', 'myworks-sync-for-xero'); ?>
										</td>
									</tr>
								...
								<table class="wqam_tbl">
									...
									<tr>
										<td colspan="2">
											...
											&nbsp;
											<?php esc_html_e('Only apply to unmapped variations', 'myworks-sync-for-xero'); ?>
										</td>
									</tr>
