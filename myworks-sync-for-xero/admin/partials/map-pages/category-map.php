...
if ( ! empty( $_POST ) && check_admin_referer( 'myworks_wc_xero_sync_map_wc_xero_category', 'map_wc_xero_category' ) ) {    
    ...
    $wpdb->query($wpdb->prepare("DELETE FROM `" . esc_sql($table) . "` WHERE `X_P_ID` = %s AND `X_ACC_CODE` = %s", '', ''));

    // Clear dashboard cache after mapping changes
    delete_transient( MyWorks_WC_Xero_Sync_Wc_Data_List::DASHBOARD_CACHE_KEY );

    // Track Mixpanel event
    MyWorks_Mixpanel_Integration::track_event( 'Map: Saved', array( 'map_type' => 'category' ) );
...
<div class="container map-product-responsive">
    <div class="page_title">
		<h4><?php esc_html_e( 'Category Mappings', 'myworks-sync-for-xero' );?></h4>
	</div>
						...
						<div class="col s12 m12 l12">
							<div class="myworks-wc-qbo-sync-table-responsive">
								<table class="mw-qbo-sync-settings-table menu-blue-bg menu-bg-a new-table">
									<thead>
										<tr>
											<th width="10%">&nbsp; <?php esc_html_e( 'ID', 'myworks-sync-for-xero' );?></th>
									...
									<?php else:?>
										<tr>
											<td colspan="4">
												<span class="mwxs_tnd">
													<?php esc_html_e( 'No categories found.', 'myworks-sync-for-xero' );?>
												</span>
											</td>
										</tr>
									<?php endif;?>
                                ...
                                <div class="mwqs_paginate_div mwqbd_pd">
                                    <div>Showing 1 to <?php echo esc_html($total_records)?> of <?php echo esc_html($total_records)?> entries</div>
                                </div>
                                <?php endif;?>
