...
if ( ! empty( $_POST ) && check_admin_referer( 'myworks_wc_xero_sync_map_wc_xero_product', 'map_wc_xero_product' ) ) {	
	...
	$wpdb->query($wpdb->prepare("DELETE FROM `" . esc_sql($table) . "` WHERE `X_P_ID` = %s", ''));
	$wpdb->query($wpdb->prepare("DELETE FROM `" . esc_sql($m_table) . "` WHERE `wc_type` = %s AND `x_type` = %s AND `x_id` = %s", 'product', 'account', ''));

	// Clear dashboard cache after mapping changes
	delete_transient( MyWorks_WC_Xero_Sync_Wc_Data_List::DASHBOARD_CACHE_KEY );

	// Track Mixpanel event
	MyWorks_Mixpanel_Integration::track_event( 'Map: Saved', array( 'map_type' => 'product' ) );
...
$xaa =  $MWXS_L->x_account_dd_type_filter($xaa,['Revenue','Sales']);
...
<div class="container map-product-responsive">
	<div class="page_title">
		<h4><?php esc_html_e( 'Product Mappings', 'myworks-sync-for-xero' );?></h4>
	</div>
	...
	<div class="mw_wc_filter">
		...
		<span class="search_text"><?php esc_html_e( 'Product Type', 'myworks-sync-for-xero' );?></span>
		&nbsp;
		<select id="product_type_srch" style="width:200px !important;">
			<option value=""><?php esc_html_e( 'All but parent variable products', 'myworks-sync-for-xero' );?></option>
						...
						<div class="col s12 m12 l12">
							<div class="myworks-wc-qbo-sync-table-responsive">
								<table class="mw-qbo-sync-settings-table menu-blue-bg menu-bg-a new-table">
									<thead>
										<tr>
											<th width="6%">&nbsp; <?php esc_html_e( 'ID', 'myworks-sync-for-xero' );?></th>
											<th width="34%">WooCommerce Product</th>
											<th width="14%"><?php esc_html_e( 'SKU', 'myworks-sync-for-xero' );?></th>
											<th width="12%"><?php esc_html_e( 'Type', 'myworks-sync-for-xero' );?></th>
											<th width="17%" class="title-description mwxs_tsns">
												<?php esc_html_e( 'Xero Product', 'myworks-sync-for-xero' );?>
											</th>
											<th width="17%" class="title-description mwxs_tsns">
												<?php esc_html_e( 'Xero Account', 'myworks-sync-for-xero' );?>
											</th>
										</tr>
									</thead>
									...
									<?php foreach($wc_product_list as $data):?>
										<tr>
											...
											<td><?php echo esc_html($data['wc_product_type'])?></td>
											<?php												
												$dd_ext_class = '';												
												if($is_ajax_dd){
											...
											<td>
												<select class="mw_wc_qbo_sync_select2 <?php echo esc_attr($dd_ext_class);?>" name="map_product_<?php echo esc_attr($data['ID'])?>" id="map_product_<?php echo esc_attr($data['ID'])?>">													
													<?php 
														if($is_ajax_dd){
															if(!empty($data['X_ItemID'])){
																echo '<option value="'.esc_attr($data['X_ItemID']).'">'.esc_html(stripslashes($data['X_Name'])).'</option>';
															}else{
									...
									<?php else:?>
										<tr>
											<td colspan="6">
												<span class="mwxs_tnd">
													<?php esc_html_e( 'No products found.', 'myworks-sync-for-xero' );?>
												</span>
											</td>
										</tr>
									<?php endif;?>
...
<script type="text/javascript">
	function search_item(){
		...
		if(product_map_search!='' || product_type_srch!='' || product_um_srch!=''){
			...
			alert('<?php echo esc_js(__('Please enter or search term.','myworks-sync-for-xero'))?>');
		}
	}
	...
	jQuery(document).ready(function($){		
		...
		$('.glp_rxp').click(function(e) {
			...
			if(!confirm('<?php echo esc_js(__('This will update the data in our sync with the latest Products in your Xero company. No data will be synced at this time.','myworks-sync-for-xero'))?>')){
				return false;
			}
		...
		$('#mwqs_automap_products_wf_qf').click(function(){
			...
			if(pam_wf!='' && pam_qf!=''){
				$('#pam_wqf_e_msg').html('');
				if(confirm('<?php echo esc_js(__('This will override any previous product mappings, and scan your WooCommerce & Xero products by selected fields to automatically match them for you.','myworks-sync-for-xero'))?>')){
					var data = {
