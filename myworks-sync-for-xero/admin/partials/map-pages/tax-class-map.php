...
// Connect to Xero and fetch tax rates for both display and save operations
$MWXS_L->xero_connect();
...
if ( ! empty( $_POST ) && check_admin_referer( 'myworks_wc_xero_sync_map_wc_xero_tax', 'map_wc_xero_tax' ) ) {
	...
	if(count($item_ids)){
		foreach ($item_ids as $key=>$value){
			...
			// Look up and store tax name
			$tax_name = '';
			if (isset($xta) && is_array($xta) && isset($xta[$value])) {
				$tax_name = $xta[$value];
			}
			$save_data['xero_tax_name'] = $tax_name;

			$save_data['wc_tax_id_2'] = 0;				
			
			//Update
			$eq = $wpdb->prepare("SELECT `id` FROM `" . esc_sql($table) . "` WHERE `wc_tax_id` = %d AND `wc_tax_id_2` = %d", $save_data['wc_tax_id'], $save_data['wc_tax_id_2']);
	...
	MyWorks_Mixpanel_Integration::track_event( 'Map: Saved', array( 'map_type' => 'tax-class' ) );
...
// Get total count with proper prepared statement
if($tax_map_search != ''){
	$search_like = '%' . $wpdb->esc_like($tax_map_search) . '%';
	$total_records = $wpdb->get_var($wpdb->prepare(
		"SELECT COUNT(*) FROM `" . esc_sql($wtr_t) . "` tr LEFT JOIN `" . esc_sql($wtr_lt) . "` trl ON (tr.tax_rate_id = trl.tax_rate_id AND trl.location_type = 'city') WHERE tr.`tax_rate_id` > 0 AND (tr.`tax_rate_name` LIKE %s OR tr.`tax_rate_class` LIKE %s OR trl.`location_code` LIKE %s)",
		$search_like,
		$search_like,
		$search_like
	));
	...
	$total_records = $wpdb->get_var($wpdb->prepare(
		"SELECT COUNT(*) FROM `" . esc_sql($wtr_t) . "` tr LEFT JOIN `" . esc_sql($wtr_lt) . "` trl ON (tr.tax_rate_id = trl.tax_rate_id AND trl.location_type = 'city') WHERE tr.`tax_rate_id` > 0"
	));
}
...
if($tax_map_search != ''){
	$search_like = '%' . $wpdb->esc_like($tax_map_search) . '%';
	$tax_q = $wpdb->prepare(
		"SELECT tr.* , trl.location_code FROM `" . esc_sql($wtr_t) . "` tr LEFT JOIN `" . esc_sql($wtr_lt) . "` trl ON (tr.tax_rate_id = trl.tax_rate_id AND trl.location_type = 'city') WHERE tr.`tax_rate_id` > 0 AND (tr.`tax_rate_name` LIKE %s OR tr.`tax_rate_class` LIKE %s OR trl.`location_code` LIKE %s) ORDER BY tr.`tax_rate_class` ASC LIMIT %d, %d",
		$search_like,
		$search_like,
		$search_like,
		$offset,
	...
	$tax_q = $wpdb->prepare(
		"SELECT tr.* , trl.location_code FROM `" . esc_sql($wtr_t) . "` tr LEFT JOIN `" . esc_sql($wtr_lt) . "` trl ON (tr.tax_rate_id = trl.tax_rate_id AND trl.location_type = 'city') WHERE tr.`tax_rate_id` > 0 ORDER BY tr.`tax_rate_class` ASC LIMIT %d, %d",
		$offset,
		$items_per_page
	);
}
...
<div class="container map-tax-class-outer map-product-responsive">
	<div class="page_title flex-box">
		<h4><?php esc_html_e( 'Tax Mappings', 'myworks-sync-for-xero' );?></h4>
	...
	<div class="mw_wc_filter">
		<span class="search_text"><?php esc_html_e( 'Search', 'myworks-sync-for-xero' );?></span>
		&nbsp;
								...
								<table class="mw-qbo-sync-map-table menu-blue-bg" width="100%">
									<thead>
										<tr>
											<th width="5%" class="title-description" id="th_id">
												<?php esc_html_e( 'ID', 'myworks-sync-for-xero' );?>							    	
											</th>
											
	                                        <th width="25%" class="title-description" id="th_tn">
												<?php esc_html_e( 'Tax Name', 'myworks-sync-for-xero' );?>
	                                        </th>
											
	                                        <th width="10%" class="title-description" id="th_tc">
	                                            <?php esc_html_e( 'Tax Class', 'myworks-sync-for-xero' );?>	
	                                        </th>
											
											<th width="10%" class="title-description" id="th_ct">
	                                            <?php esc_html_e( 'City', 'myworks-sync-for-xero' );?>
	                                        </th>
											
	                                        <th width="10%" class="title-description" id="th_cn">
	                                            <?php esc_html_e( 'Country', 'myworks-sync-for-xero' );?>
	                                        </th>
											
	                                        <th width="10%" class="title-description" id="th_st">
	                                            <?php esc_html_e( 'State', 'myworks-sync-for-xero' );?>
	                                        </th>
											
	                                        <th width="10%" class="title-description" id="th_rt">
	                                            <?php esc_html_e( 'Rate', 'myworks-sync-for-xero' );?>
	                                        </th>
											
	                                        <th width="20%" class="title-description" id="th_qt">
	                                            <?php esc_html_e( 'Xero Tax', 'myworks-sync-for-xero' );?>
	                                        </th>
										</tr>
									</thead>
									...
									<?php else:?>
										<tr>
											<td colspan="8">
												<span class="mwxs_tnd">
													<?php esc_html_e( 'No taxes found.', 'myworks-sync-for-xero' );?>
												</span>
											</td>
										</tr>
									<?php endif;?>
