...
if ( ! empty( $_POST ) && check_admin_referer( 'myworks_wc_xero_sync_map_wc_xero_variation', 'map_wc_xero_variation' ) ) {	
	...
	$wpdb->query($wpdb->prepare("DELETE FROM `" . esc_sql($m_table) . "` WHERE `wc_type` = %s AND `x_type` = %s AND `x_id` = %s", 'variation', 'account', ''));

	// Clear dashboard cache after mapping changes
	delete_transient( MyWorks_WC_Xero_Sync_Wc_Data_List::DASHBOARD_CACHE_KEY );

	// Track Mixpanel event
	MyWorks_Mixpanel_Integration::track_event( 'Map: Saved', array( 'map_type' => 'variation' ) );
...
<div class="container map-product-responsive">
	<div class="page_title">
		<h4><?php esc_html_e( 'Variation Mappings', 'myworks-sync-for-xero' );?></h4>
	</div>
								...
								<table class="mw-qbo-sync-settings-table menu-blue-bg menu-bg-a new-table">
									<thead>
										<tr>
											...
											<th width="34%"><?php esc_html_e( 'WooCommerce Variation', 'myworks-sync-for-xero' );?></th>
											<th width="14%"><?php esc_html_e( 'Variation SKU', 'myworks-sync-for-xero' );?></th>
											<th width="12%"><?php esc_html_e( 'Parent Product', 'myworks-sync-for-xero' );?></th>
										...
										<tr>
											...
											<td>
												<a href="<?php echo esc_url(admin_url('post.php?action=edit&post=').(int) $data['parent_id']) ?>" target="_blank">
													<b><?php echo esc_html($data['name']);?></b>
														...
														if($is_a_k_v_c){
															...
															if(is_array($attr_arr) && count($attr_arr)){
																...
																foreach($attr_arr as $key=>$val){							
																	...
																	echo esc_html(ucfirst($key)).': '.esc_html($val).'<br>';
																}
															}
														}													
													}
									...
									<?php else:?>
										<tr>
											<td colspan="6">
												<span class="mwxs_tnd">
													<?php esc_html_e( 'No variations found.', 'myworks-sync-for-xero' );?>
												</span>
											</td>
										</tr>
									<?php endif;?>
...
<script type="text/javascript">
	...
	jQuery(document).ready(function($){		
		...
		$('#mwqs_automap_variations_wf_qf').click(function(){
			...
			if(vam_wf!='' && vam_qf!=''){
				$('#vam_wqf_e_msg').html('');
				if(confirm('<?php echo esc_js(__('This will override any previous variation mappings, and scan your WooCommerce & Xero products by selected fields to automatically match them for you.','myworks-sync-for-xero'))?>')){
					var data = {
