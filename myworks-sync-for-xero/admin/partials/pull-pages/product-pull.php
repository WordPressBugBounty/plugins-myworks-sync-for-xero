...
<?php echo wp_kses(str_repeat('<br>',2), ['br' => []]);?>
<div class="container">
	<div class="page_title"><h4><?php esc_html_e( 'Product Pull', 'myworks-sync-for-xero' );?></h4></div>
					...
					<div class="row">
						<div class="input-field col s12 m12 14">
							<button id="pull_selected_product_btn" class="waves-effect waves-light btn save-btn mw-qbo-sync-green">
								<?php echo esc_html__('Pull Selected Products','myworks-sync-for-xero')?>
							</button>										
						</div>
					</div>
					<br>
							...
							<table class="table tablesorter" id="mwqs_product_pull_table">
								...
								<tbody>
									...
									<tr>
										...
										<td><?php echo esc_html($Item->getCode()) ?></td>
										<td>
											<?php 
										...
										<td><?php echo esc_html($MWXS_L->format_xero_date($Item->getUpdatedDateUTC()))?></td>
											...
											<?php 
												if(is_array($mapped_data) && !empty($mapped_data)){
													...
													if(isset($mapped_data['mapped'])){
														echo '<i title="Mapped to '.esc_attr($wpv).' #'.esc_attr($mapped_data['ID']).'" class="fa fa-check-circle" style="color:green"></i>';
													}else{
														echo '<i title="Mached (SKU / Name) to '.esc_attr($wpv).' #'.esc_attr($mapped_data['ID']).'" class="fa fa-check-circle" style="color:green"></i>';
													}
