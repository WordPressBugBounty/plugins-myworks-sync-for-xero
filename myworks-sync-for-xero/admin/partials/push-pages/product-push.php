...
<div class="container">
	<div class="page_title"><h4><?php esc_html_e( 'Product Push', 'myworks-sync-for-xero' );?></h4></div>
					...
					<div class="row">
						<div class="input-field col s12 m12 14">
							<button id="push_selected_product_btn" class="waves-effect waves-light btn save-btn mw-qbo-sync-green">
								<?php echo esc_html__('Push Selected Products','myworks-sync-for-xero')?>
							</button>										
						</div>
					</div>
					<br>
							...
							<table class="table tablesorter" id="mwqs_product_push_table">
								...
								<tbody>
									...
									<tr>
										...
										<td>
											<a href="<?php echo esc_url(admin_url('post.php?action=edit&post=').(int) $data['ID']) ?>" target="_blank"><?php echo esc_html($data['name']);?></a>
										</td>
										...
										<td><?php echo esc_html($data['manage_stock']);?></td>
										<td><?php echo number_format(floatval($data['stock']),2);?></td>
										<td><?php echo esc_html($data['wc_product_type']);?></td>
										<td><?php echo esc_html($data['stock_status']);?></td>
										<td><?php echo esc_html($data['total_sales']);?></td>
										<td>
											<span class="ss_pf_span"><?php echo (!empty($data['X_ItemID']))?1:0;?></span>											
											<?php 
												if(!empty($data['X_ItemID'])){
													echo '<i title="Mapped to '.esc_attr($data['X_Name']).'" class="fa fa-check-circle" style="color:green"></i>';
												}else{
