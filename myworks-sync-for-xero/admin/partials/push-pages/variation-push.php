...
<div class="container">
	<div class="page_title"><h4><?php esc_html_e( 'Variation Push', 'myworks-sync-for-xero' );?></h4></div>
					...
					<div class="row">
						<div class="input-field col s12 m12 14">
							<button id="push_selected_variation_btn" class="waves-effect waves-light btn save-btn mw-qbo-sync-green">
								<?php echo esc_html__('Push Selected Variations','myworks-sync-for-xero')?>
							</button>										
						</div>
					</div>
					<br>
							...
							<table class="table tablesorter" id="mwqs_variation_push_table">
								...
								<tbody>
									...
									<tr>
										...
										<td>
											<?php 
												echo esc_html($MWXS_L->get_variation_name_from_id($data['name'],$data['parent_name'],$data['ID'],$data['parent_id']));
											?>
										</td>
										...
										<td>
											<a title="<?php echo (int) $data['parent_id']?>" target="_blank" href="<?php echo esc_url_raw(admin_url('post.php?action=edit&post=').(int) $data['parent_id']) ?>">
												<?php echo esc_html($data['parent_name']);?>
											</a>
										</td>
