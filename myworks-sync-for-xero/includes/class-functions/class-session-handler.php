<?php
// phpcs:disable WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Custom session table management
...
class MyWorks_WC_Xero_Sync_Lib_Session_Handler extends WC_Session {
	...
	protected function use_secure_cookie() {
		// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- Using WooCommerce core filter, not a plugin-defined hook
	...
	public function set_session_expiration() {
		// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- mw_wc_xero_sync is the established legacy prefix for backward compatibility
	...
	public function save_data( $old_session_key = 0 ) {
		...
		if ( $this->_dirty && $this->has_session() ) {
			...
			$wpdb->query($wpdb->prepare("INSERT INTO `" . esc_sql($this->_table) . "` (`session_key`, `session_value`, `session_expiry`) VALUES (%s, %s, %d) ON DUPLICATE KEY UPDATE `session_value` = VALUES(`session_value`), `session_expiry` = VALUES(`session_expiry`)", $this->_customer_id, maybe_serialize( $this->_data ), $this->_session_expiration)); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
	...
	public function cleanup_sessions() {
		...
		$wpdb->query( $wpdb->prepare( "DELETE FROM `" . esc_sql($this->_table) . "` WHERE session_expiry < %d", time() ) ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
	...
	public function get_session( $customer_id, $default = false ) {
		...
		if ( false === $value ) {
			$value = $wpdb->get_var( $wpdb->prepare( "SELECT session_value FROM `" . esc_sql($this->_table) . "` WHERE session_key = %s", $customer_id ) ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
