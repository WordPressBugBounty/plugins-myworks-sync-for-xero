<?php
// phpcs:disable WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Plugin activation requires direct database table creation
...
class MyWorks_WC_Xero_Sync_Activator {
	...
	public static function activate() {
		...
		if(!$is_activate_plugin){
			die(wp_kses($activation_error_msg, ['a' => ['href' => [], 'target' => []]]));
		}

		Self::create_plugin_database_tables();
		Self::add_plugin_default_setting_options();
		Self::pulse_db_migration();
		Self::do_after_activation();
	}

	/**
	 * Pulse database migration
	 * Add columns for tracking Pulse sent status
	 *
	 * @since 1.3.2
	 */
	protected static function pulse_db_migration() {
		// Require Pulse integration class
		require_once plugin_dir_path( __FILE__ ) . 'class-myworks-pulse-integration.php';
		MyWorks_Pulse_Integration::db_migration();
	}
	...
	protected static function create_plugin_database_tables(){
		...
		// Security: Escape table prefix even though it's a constant
		$safe_dtp = esc_sql($dtp);
		
		#Xero Customer
		$sql[] = "CREATE TABLE IF NOT EXISTS {$safe_dtp}customers (
		...
		#Customer Map
		$sql[] = "CREATE TABLE IF NOT EXISTS {$safe_dtp}map_customers (
			`id` int(11) NOT NULL AUTO_INCREMENT,
		...
		#Xero Product
		$sql[] = "CREATE TABLE IF NOT EXISTS {$safe_dtp}products (
			`id` int(11) NOT NULL AUTO_INCREMENT,
		...
		#Product Map
		$sql[] = "CREATE TABLE IF NOT EXISTS {$safe_dtp}map_products (
			`id` int(11) NOT NULL AUTO_INCREMENT,
		...
		#Variation Map
		$sql[] = "CREATE TABLE IF NOT EXISTS {$safe_dtp}map_variations (
			`id` int(11) NOT NULL AUTO_INCREMENT,
		...
		# Category Map
		$sql[] = "CREATE TABLE IF NOT EXISTS {$safe_dtp}map_categories (
			`id` int(11) NOT NULL AUTO_INCREMENT,
		...
		#Payment Method Map
		$sql[] = "CREATE TABLE IF NOT EXISTS {$safe_dtp}map_payment_method (
			`id` int(11) NOT NULL AUTO_INCREMENT,
			`wc_payment_method` varchar(255) NOT NULL,
			`currency` char(3) NOT NULL,
			`enable_payment` int(1) NOT NULL,
			`X_ACC_ID` varchar(36) NOT NULL,
			`xero_account_name` varchar(255) NOT NULL DEFAULT '',
		...
		#Tax Map
		$sql[] = "CREATE TABLE IF NOT EXISTS {$safe_dtp}map_tax (
			`id` int(11) NOT NULL AUTO_INCREMENT,
			`wc_tax_id` int(11) NOT NULL,
			`xero_tax` varchar(255) NOT NULL,
			`xero_tax_name` varchar(255) NOT NULL DEFAULT '',
			`wc_tax_id_2` int(11) NOT NULL,
			PRIMARY KEY (id)
		)";
		
		#Plugin Log
		$sql[] = "CREATE TABLE IF NOT EXISTS {$safe_dtp}log (
			`id` int(11) NOT NULL AUTO_INCREMENT,
		...
		#Plugin Queue
		$sql[] = "CREATE TABLE IF NOT EXISTS `{$safe_dtp}queue` (
			`id` int(11) NOT NULL AUTO_INCREMENT,
		...
		#Plugin Session
		$sql[] = "CREATE TABLE IF NOT EXISTS {$safe_dtp}sessions (
		...
		# Map Multiple
		$sql[] = "CREATE TABLE IF NOT EXISTS {$safe_dtp}map_multiple (
			`id` int(11) NOT NULL AUTO_INCREMENT,
		...
		# Custom Field Map
		$sql[] = "CREATE TABLE IF NOT EXISTS {$safe_dtp}map_custom_fields (
			`id` int(11) NOT NULL AUTO_INCREMENT,
		...
		if(is_array($sql) && !empty($sql)){
			foreach($sql as $query){
				// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- CREATE TABLE queries cannot use prepared statements. Queries are generated by dbDelta() which is WordPress core.
				$wpdb->query($query);
			}
		}
		...
		// Add xero_account_name column to map_payment_method if it doesn't exist
		$payment_table = $safe_dtp . 'map_payment_method';
		$column_check = $wpdb->get_results(
			$wpdb->prepare("SHOW COLUMNS FROM `" . esc_sql($payment_table) . "` LIKE %s", 'xero_account_name')
		);
		if (empty($column_check)) {
			$alter_sql[] = "ALTER TABLE `" . esc_sql($payment_table) . "` ADD COLUMN `xero_account_name` varchar(255) NOT NULL DEFAULT '' AFTER `X_ACC_ID`";
		}

		// Add xero_tax_name column to map_tax if it doesn't exist
		$tax_table = $safe_dtp . 'map_tax';
		$column_check = $wpdb->get_results(
			$wpdb->prepare("SHOW COLUMNS FROM `" . esc_sql($tax_table) . "` LIKE %s", 'xero_tax_name')
		);
		if (empty($column_check)) {
			$alter_sql[] = "ALTER TABLE `" . esc_sql($tax_table) . "` ADD COLUMN `xero_tax_name` varchar(255) NOT NULL DEFAULT '' AFTER `xero_tax`";
		}

		// Run ALTER TABLE queries
		if (!empty($alter_sql)) {
			foreach($alter_sql as $query) {
				// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- ALTER TABLE queries for schema updates
				$wpdb->query($query);
			}
		}
	}
