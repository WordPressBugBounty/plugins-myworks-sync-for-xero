...
class ConsumerStrategies_CurlConsumer extends ConsumerStrategies_AbstractConsumer {
    ...
    protected function _execute($url, $batch) {
        ...
        for ($i=0; $i<$this->_num_threads && !empty($batch); $i++) {
            ...
            // Windows PHP ships with no CA trust store, so TLS verification fails
            // without an explicit bundle; use the one WordPress ships.
            // (PHP_OS, not PHP_OS_FAMILY: this plugin supports PHP 5.6.)
            if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN' && defined('ABSPATH')) {
                $wp_ca_bundle = ABSPATH . WPINC . '/certificates/ca-bundle.crt';
                if (is_readable($wp_ca_bundle)) {
                    curl_setopt($ch, CURLOPT_CAINFO, $wp_ca_bundle);
                }
            }
