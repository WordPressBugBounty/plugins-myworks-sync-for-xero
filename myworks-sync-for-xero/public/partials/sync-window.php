...
$ext_options = array();
// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Public sync window display option, read-only parameter
...
<html>
	<head>
		<title><?php echo esc_html__('Xero Sync Progress','myworks-sync-for-xero');?></title>
		<?php 
		// Properly enqueue styles and scripts
		wp_enqueue_style( 'bootstrap-min', plugin_dir_url( dirname(dirname(__FILE__)) ) . 'admin/css/bootstrap.min.css', array(), '1.0.0' );
		wp_enqueue_style( 'font-awesome', plugin_dir_url( dirname(dirname(__FILE__)) ) . 'admin/css/font-awesome.css', array(), '1.0.0' );
		wp_enqueue_style( 'sync-window', plugin_dir_url( dirname(__FILE__) ) . 'css/myworks-wc-xero-sync-window.css', array(), '1.0.0' );
		wp_enqueue_script( 'jquery' );
		wp_enqueue_script( 'bootstrap-min', plugin_dir_url( dirname(dirname(__FILE__)) ) . 'admin/js/bootstrap.min.js', array('jquery'), '1.0.0', true );
		wp_head();
		?>
	...
	<body <?php if($is_valid_sync){echo 'onload="start_ajax_push();"';}?>>
		...
		<div class="sw_div">
			...
			<h5><?php echo esc_html__('Total','myworks-sync-for-xero');?>: <?php echo esc_html($tot);?></h5>
			...
			<div class="sw_process">
				<b id="process_txt"><?php echo esc_html__('Processing','myworks-sync-for-xero');?>:</b>
				&nbsp;
			...
			<div class="sw_close">
				...
				<button id="close_btn_id" class="btn btn-default right-btn" onclick="javascript:stop_ajax();self.close ();"><?php echo esc_html__('Close','myworks-sync-for-xero');?></button>
		...
		<?php else:?>
		<h1><?php echo esc_html__('Error','myworks-sync-for-xero');?>!</h1>
		<?php endif;?>
	</body>
</html>